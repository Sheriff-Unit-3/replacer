...
if (core.get_modpath("trees")
	...
	and core.get_modpath("scribing_table")) then
...
replacer.inspect = function(_, user, pointed_thing, mode, show_receipe)
	...
	if (pointed_thing.type == 'object') then
		...
		elseif (ref:is_player()) then
			...
			if( luaob and luaob.get_staticdata) then
				...
				if (0 < #sdata) then
					sdata = core.deserialize(sdata) or {}
		...
		text = text..' at '..core.pos_to_string(ref:getpos())
		core.chat_send_player(name, text)
		return nil
	elseif (pointed_thing.type ~= 'node') then
		core.chat_send_player(name, 'Sorry. This is an unkown something of type \"'..
	...
	if (not(core.registered_nodes[node.name])) then
		...
		text = 'This is a \"'..tostring(core.registered_nodes[node.name].description or
...
replacer.add_circular_saw_receipe = function(node_name, receipes)
	if (not(node_name) or not(core.get_modpath("moreblocks")) or not(circular_saw) or not(circular_saw.names) or (node_name=='moreblocks:circular_saw')) then
		return
	end
...
replacer.inspect_show_crafting = function(name, node_name, fields)
	...
	if (fields) then
		for k,v in pairs(fields) do
			if (v and v=="" and (core.registered_items[k]
					 or	core.registered_nodes[k]
					 or	core.registered_craftitems[k]
					 or	core.registered_tools[k])) then
				node_name = k
				receipe_nr = 1
			end
		end
	...
	local res = core.get_all_craft_recipes(node_name)
	if (not(res)) then
		res = {}
	end
	...
	if (	 core.registered_nodes[node_name]) then
		if (	 core.registered_nodes[node_name].description
			and core.registered_nodes[node_name].description~= "") then
			desc = "\""..core.registered_nodes[node_name].description.."\" block"
		elseif (core.registered_nodes[node_name].name) then
			desc = "\""..core.registered_nodes[node_name].name.."\" block"
		else
			desc = " - no description provided - block"
		end
	elseif (core.registered_items[node_name]) then
		if (	 core.registered_items[node_name].description
			and core.registered_items[node_name].description~= "") then
			desc = "\""..core.registered_items[node_name].description.."\" item"
		elseif (core.registered_items[node_name].name) then
			desc = "\""..core.registered_items[node_name].name.."\" item"
		else
	...
	local formspec = "size[6,6]"..
		"label[0,5.5;This is a "..core.formspec_escape(desc)..".]"..
	...
	if (fields.pos) then
		formspec = formspec.."label[0.0,0.3;Located at "..
			core.formspec_escape(core.pos_to_string(fields.pos))
	...
	if (fields.protected_info and fields.protected_info ~= "") then
		formspec = formspec.."label[0.0,4.5;"..core.formspec_escape(fields.protected_info).."]"
	end
	...
	if (not(res) or #res<1) then
		...
		if (core.registered_nodes[node_name]
			and core.registered_nodes[node_name].drop) then
	...
	core.show_formspec(name, "replacer:crafting", formspec)
end
